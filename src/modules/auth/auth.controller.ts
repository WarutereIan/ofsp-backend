...
// Cookie configuration for cross-origin auth (frontend and backend on different domains, e.g. vercel.app and jirani.store)
// Cross-domain requires: SameSite=None, Secure=true (HTTPS). partitioned enables CHIPS for mobile browsers.
// Set CROSS_DOMAIN_COOKIES=true (e.g. in .env) to use cross-domain options in non-production (backend must use HTTPS).
const isProduction = process.env.NODE_ENV === 'production' ;
const useCrossDomainCookies =
  isProduction || process.env.CROSS_DOMAIN_COOKIES === 'true';
const COOKIE_OPTIONS: {
  ...
  httpOnly: true,
  path: '/',
  // Cross-domain: sameSite 'none' + secure true so cookies are sent cross-site; partitioned for CHIPS (mobile)
  secure: useCrossDomainCookies,
  sameSite: useCrossDomainCookies ? ('none' as const) : ('lax' as const),
  ...(useCrossDomainCookies && { partitioned: true }),
};
